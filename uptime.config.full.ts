...
const pageConfig: PageConfig = {
  ...
  title: "allovend's Status Page",
  // Links shown at the header of your status page, could set `highlight` to `true`
  links: [
    { link: 'https://github.com/allovend', label: 'GitHub' },
    { link: '3126385134@qq.com', label: 'Email Me', highlight: true },
  ],
  // [OPTIONAL] Group your monitors
  // If not specified, all monitors will be shown in a single list
  // If specified, monitors will be grouped and ordered, not-listed monitors will be invisble (but still monitored)
  group: {
    '🌐 Public (example group name)': ['openlist', 'more monitor ids...'],
    //'🔐 Private': [''],
  },
...
const workerConfig: WorkerConfig = {
  ...
  monitors: [
    // Example HTTP Monitor
    {
      ...
      target: 'https://defendsvip-openlist.hf.space',
      // [OPTIONAL] `tooltip` is ONLY used at status page to show a tooltip
      tooltip: '检查Defendsvip Openlist Huggingface Space 是否可访问',
      // [OPTIONAL] `statusPageLink` is ONLY used for clickable link at status page
      statusPageLink: 'https://defendsvip-openlist.hf.space',
  ...
  notification: {
    ...
    // More info at Wiki: https://github.com/allovend/UptimeFlare/wiki/Setup-notification
    webhook: {
    ...
    skipNotificationIds: ['openlist_hf_space'],
...
const maintenances: MaintenanceConfig[] = [
  {
    ...
    monitors: ['openlist_hf_space'],
    // [Optional] default to "Scheduled Maintenance" if not specified
    title: '维护测试',
    // Description of the maintenance, will be shown at status page
    body: '这是一次测试维护，服务器软件升级',
  ...
  ...(function () {
    ...
    for (let i = -1; i <= 1; i++) {
      ...
      schedules.push({
        title: `${year}/${parseInt(month)} - 测试定期维护`,
        monitors: ['openlist_hf_space'],
